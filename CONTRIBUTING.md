# Contributing to eslint-config

eslint-config has been created to fulfill the National Bank of Belgium's front-end development needs, but is now an open source project.

As such, third party contributions are more than welcome! Here you'll find all the information you need to get started.

We would love for you to contribute to eslint-config and make it better and better.
As a contributor, here are the guidelines we would like you to follow.

## Code of Conduct

Help us keep eslint-config open and inclusive. Please read and follow our [Code of Conduct](/CODE_OF_CONDUCT.md).

## Got a question or problem?

Ask questions on [Stack Overflow](https://stackoverflow.com/questions/tagged/nbbEslintConfig) where the questions should be tagged with tag `nbbEslintConfig`
...
Take a look at the project's open [issues](https://github.com/NationalBankBelgium/eslint-config/issues) and [milestones](https://github.com/NationalBankBelgium/eslint-config/issues/milestones).
...
- version of eslint-config & linting tool used
...
You can file new issues by filling out our [new issue form](https://github.com/NationalBankBelgium/eslint-config/issues/new).

### Forking

eslint-config development is done using a forking model with Pull Requests (PRs), so the very first thing you should do is create your fork: https://help.github.com/articles/fork-a-repo/
...
Once you're found what you want to contribute to eslint-config, then:

- Create a feature branch in your fork: `git checkout -b my-new-branch master`
- Configure the upstream
  - `git remote add upstream https://github.com/NationalBankBelgium/eslint-config.git`
...
Once done, you may submit a new Pull Request (PR): https://github.com/NationalBankBelgium/eslint-config/pull/new/master
...
Here's a summary of the workflow we recommend to contribute to eslint-config along with the commands to use. Note that this only uses a subset of what git flow has to offer.
...
- fork: through GitHub
- clone: `git clone https://github.com/<username>/eslint-config`
- add upstream: `git remote add upstream https://github.com/NationalBankBelgium/eslint-config`
  - that way you can fetch new commits that have been integrated into eslint-config's main repository
...
- add the other developer's fork as a remote: `git remote add <username> https://github.com/<username>/eslint-config`
...
- make changes
- push the changes
- sync your local repo with the main repository of eslint-config
- create PRs against the main repository of eslint-config (or let the other developer do it)

In any case, after PRs are merged into eslint-config, you can normally forget about the feature branches and delete those to keep your fork clean
...
If you want to build eslint-config from source, you need to...

- install NodeJS (14.20.0+) and npm (8.19.1+)
- clone this git repository: `git clone https://github.com/NationalBankBelgium/eslint-config.git`
- run `npm run install` from the root directory of eslint-config

### Executing eslint-config's main build script

Anytime you make modifications to the eslint-config library, you'll need to

- execute `npm run build` from the root directory of eslint-config

That command will then build eslint-config using the [build script](/build.sh).

### Hacking the eslint-config library

If you want to modify the eslint-config library:

- go to the package folder. `cd eslint-config`
- install its dependencies: `npm install`
- make your changes
- execute `npm run build` at eslint-config's root
...
You can execute the tests for the eslint-config library using the following command at the root: `npm test`
